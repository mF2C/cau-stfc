...
# It is intended to run in the CAU container.
...
#convert to p12
openssl pkcs12 -export -in "${fp}"cau.pem -inkey "${fp}"cau.pem -out "${fp}"cau.p12 -name server -CAfile "${fp}"trust.pem -caname root -passin pass:serverStore -passout pass:serverStore

if [ ! -e "${fp}"cau.p12 ]; then
...
#keystore
keytool -importkeystore -srckeystore "${fp}"cau.p12 -srcstoretype pkcs12 -destkeystore "${fp}"cau-server.jks -deststoretype JKS -storepass serverStore -keypass serverStore -srcstorepass serverStore -noprompt
...
keytool -import -file "${fp}"untrust.pem -alias it2ut -keystore clientTrust.jks -storepass trustStore -noprompt
